...
DECLARE @SCHEMA_NAME nvarchar(32),
		...
		@CREATE_MESSAGE_ID_SEQUENCE_DDL nvarchar(1000);
...
SET @CREATE_MESSAGE_ID_SEQUENCE_DDL =
N'CREATE SEQUENCE [' + @SCHEMA_NAME + N'].[@SequenceName]
    AS [BIGINT]
	START WITH 1
	INCREMENT BY 1
	MINVALUE -2147483648
			...
			BEGIN
				...
				WHILE @counter < @MESSAGE_TABLE_COUNT
					BEGIN
						...
						DECLARE @Sequence_name nvarchar(100);
						...
						--Create the sequence
						SET @Sequence_Name = 'SEQ_' +  @table_name
						SET @ddl = REPLACE(@CREATE_MESSAGE_ID_SEQUENCE_DDL, '@SequenceName', @Sequence_Name);
						IF NOT EXISTS(SELECT [object_id] 
									  FROM [sys].[objects] 
							...
							BEGIN
								EXEC(@ddl);
								PRINT 'Created message ID sequence [' + @SCHEMA_NAME + ']' + '.' + '[' + @Sequence_Name + ']';
							END
						ELSE
							PRINT 'Message ID sequence [' + @SCHEMA_NAME + ']' + '.' + '[' + @Sequence_Name + '] already exists';
...
ELSE -- @CURRENT_SCHEMA_VERSION > @TARGET_SCHEMA_VERSION
	BEGIN
		...
		RAISERROR(N'SignalR database current schema version %d is newer than the configured SqlMessageBus schema version %d. Please update to the latest Microsoft.AspNet.SignalR.SqlServerSequences NuGet package.', 11, 1,
